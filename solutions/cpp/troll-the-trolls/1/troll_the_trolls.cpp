...
bool permission_check(Action act, AccountStatus acc) {
    ...
    if (acc == AccountStatus::user or acc == AccountStatus::troll) {
        if (act == Action::read or act == Action::write) {
            return true;
        }
    }
    if (acc == AccountStatus::mod) {
        if (act == Action::read or act == Action::write or act == Action::remove) {
            return true;
        }
    } 
    return false;
...
bool has_priority(AccountStatus a1, AccountStatus a2) {
    if (a1 == AccountStatus::mod && a2 != AccountStatus::mod) {
        return true;
    } else if (a1 == AccountStatus::user && (a2 == AccountStatus::guest || a2 == AccountStatus::troll)) {
        return true;
    } else if (a1 == AccountStatus::guest && a2 == AccountStatus::troll) {
        return true;
    }
    return false;
}
